...
fun Streets(navController: NavController?) { // <-- Change this name for Water, Lights, etc.
    ...
    var streeData by remember { mutableStateOf(listOf<IssueItem>()) }
    var isLoading by remember { mutableStateOf(true) }

    // 2. Connect to the "issues" collection
    LaunchedEffect(Unit) {
        db.collection("Issues") // Ensure this matches your capital "I" collection
            .whereEqualTo("category", "Roads") // Filters for the "Toxic Waste" entry
            .addSnapshotListener { value, error ->
                if (error == null && value != null) {
                    streeData = value.documents.map { doc ->
                        IssueItem(
    ...
    Scaffold(
        topBar = { AdminHeader(title = "Street Report Issue") }
    ) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            if (isLoading) {
                ...
                IssueTabTemplate(
                    totalCount = streeData.size,
                    newCount = streeData.count { it.status == "Pending" },
                    issues = streeData,
