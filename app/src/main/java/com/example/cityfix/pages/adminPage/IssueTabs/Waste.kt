...
fun Waste(navController: NavController?) { // <-- Change this name for Water, Lights, etc.
    ...
    var wasteData by remember { mutableStateOf(listOf<IssueItem>()) }
    var isLoading by remember { mutableStateOf(true) }

    // 2. Connect to the "issues" collection
    LaunchedEffect(Unit) {
        db.collection("Issues") // Ensure this matches your capital "I" collection
            .whereEqualTo("category", "Waste") // Filters for the "Toxic Waste" entry
            .addSnapshotListener { value, error ->
                if (error == null && value != null) {
                    wasteData = value.documents.map { doc ->
                        IssueItem(
    ...
    Scaffold(
        topBar = { AdminHeader(title = "Waste Report Issue") }
    ) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            if (isLoading) {
                ...
                IssueTabTemplate(
                    totalCount = wasteData.size,
                    newCount = wasteData.count { it.status == "Pending" },
                    issues = wasteData,
