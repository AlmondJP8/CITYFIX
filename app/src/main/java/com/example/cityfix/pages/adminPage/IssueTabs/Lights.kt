...
fun Lights(navController: NavController?) {
    ...
    var lightData by remember { mutableStateOf(listOf<IssueItem>()) }
    var isLoading by remember { mutableStateOf(true) }

    // 2. Connect to the "issues" collection
    LaunchedEffect(Unit) {
        db.collection("Issues") // Ensure this matches your capital "I" collection
            .whereEqualTo("category", "Lights") // Filters for the "Toxic Waste" entry
            .addSnapshotListener { value, error ->
                if (error == null && value != null) {
                    lightData = value.documents.map { doc ->
                        IssueItem(
    ...
    Scaffold(
        topBar = { AdminHeader(title = "Lights Report Issue") }
    ) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            if (isLoading) {
                ...
                IssueTabTemplate(
                    totalCount = lightData.size,
                    newCount = lightData.count { it.status == "Pending" },
                    issues = lightData,
